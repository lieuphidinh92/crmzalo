...
async function fetchReport() {
  ...
  try {
    ...
    if (tab.value === 'messages') {
      ...
      for (const s of (raw.stage ?? [])) {
        rows.push({ label: `Stage: ${s.status}`, count: Number(s.count ?? 0) });
      }
      for (const c of (raw.customerType ?? [])) {
        rows.push({ label: `Loại KH: ${c.status}`, count: Number(c.count ?? 0) });
      }
