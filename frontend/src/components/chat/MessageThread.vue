<template>
  <div class="message-thread d-flex flex-column flex-grow-1" style="height: 100%;">
    ...
    <template v-else>
      ...
      <div ref="messagesContainer" class="flex-grow-1 overflow-y-auto pa-3 chat-messages-area">
        ...
        <div v-for="msg in messages" :key="msg.id" class="mb-2 d-flex" :class="msg.senderType === 'self' ? 'justify-end' : 'justify-start'">
          <div style="max-width: 70%;">
            <div v-if="conversation.threadType === 'group' && msg.senderType !== 'self'" class="text-caption mb-1" style="color: var(--brand-amber-500); font-weight: 500;">
      ...
      <!-- Quick reply chips — sit just above the input area -->
      <QuickReplyBar
...
import QuickReplyBar from '@/components/chat/QuickReplyBar.vue';
