...
export async function reportRoutes(app: FastifyInstance): Promise<void> {
  ...
  // GET /api/v1/reports/contacts?from=&to=
  app.get('/api/v1/reports/contacts', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const [newPerDay, stageDist, customerTypeDist] = await Promise.all([
        ...
        prisma.contact.groupBy({
          by: ['stage'],
          where: { orgId, stage: { not: null } },
          _count: true,
        }),
        prisma.contact.groupBy({
          by: ['customerType'],
          where: { orgId, customerType: { not: null } },
          _count: true,
        }),
      ]);
      ...
      return {
        ...
        stage: stageDist.map((s) => ({ status: s.stage, count: s._count })),
        customerType: customerTypeDist.map((c) => ({
