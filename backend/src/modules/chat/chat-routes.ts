...
export async function chatRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/conversations/:id/messages', { preHandler: requireZaloAccess('chat') }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      // Compliance hook: log a Zalo reply by this sale. Compute response
      // time vs. the most recent inbound contact-side message in the
      // conversation so the score calculator can use it cheaply.
      const lastInbound = await prisma.message.findFirst({
        where: { conversationId: id, senderType: 'contact' },
        orderBy: { sentAt: 'desc' },
        select: { sentAt: true },
      });
      const responseSeconds = lastInbound
        ? Math.round((Date.now() - lastInbound.sentAt.getTime()) / 1000)
        : null;
      logCompliance({
        orgId: user.orgId,
        saleId: user.id,
        actionType: 'zalo_replied',
